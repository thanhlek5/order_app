...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QGridLayout, 
                            ...
                            QDialog, QListWidget, QTextEdit, QComboBox)
from PyQt6.QtCore import Qt, QSize
from PyQt6.QtGui import QFont, QColor, QPalette, QIcon, QImage, QPixmap
import difflib
...
class UserManager:
    def __init__(self):
        self.users = {
            "admin": {"password": "1234", "email": "admin@coffeef5.com", "role": "admin"},
            "staff": {"password": "1111", "email": "staff@coffeef5.com", "role": "staff"}
        }
    ...
    def register(self, username, password, email):
        ...
        self.users[username] = {"password": password, "email": email, "role": "staff"}
...
class LoginWindow(QMainWindow):
    ...
    def verify_user_info(self, dialog):
        ...
        stored_email = self.user_manager.get_email(username)
...
class CoffeePOS(QMainWindow):
    def __init__(self):
        ...
        self.menu_items = {
            ...
            "Sữa Chua Lắc Phổ Mai Bột Tự": 28000
        }
    ...
    def select_suggestion(self, item):
        try:
            ...
            print(f"Error in select_suggestion: {e}")
    ...
    def filter_menu(self):
        try:
            ...
            print(f"Error in filter_menu: {e}")
    ...
    def show_print_receipt_dialog(self):
        try:
            ...
            print(f"Error in show_print_receipt_dialog: {e}")
    ...
    def print_receipt(self, dialog, payment_method):
        try:
            ...
            print(f"Error in print_receipt: {e}")
    ...
    def show_payment_dialog(self):
        try:
            ...
            print(f"Error in show_payment_dialog: {e}")
    ...
    def show_notification_dialog(self):
        try:
            ...
            print(f"Error in show_notification_dialog: {e}")
    ...
    def show_table_transfer_dialog(self):
        try:
            ...
            print(f"Error in show_table_transfer_dialog: {e}")
    ...
    def transfer_table(self, new_table, dialog):
        try:
            ...
            print(f"Error in transfer_table: {e}")
    ...
    def show_staff_management(self):
        try:
            ...
            staff_list = QTextEdit()
            staff_list.setReadOnly(True)
            staff_list.setStyleSheet("background-color: white; color: black; font-size: 14px;")
            staff_list.setText("\n".join([f"{user} ({self.user_manager.users[user]['role']}, {self.user_manager.users[user]['email']})" for user in self.user_manager.users.keys()]))
            layout.addWidget(staff_list)
            
            button_widget = QWidget()
            button_layout = QHBoxLayout(button_widget)
            
            add_button = QPushButton("Thêm Nhân Viên")
            ...
            """)
            add_button.clicked.connect(self.show_add_staff_dialog)
            ...
            print(f"Error in show_staff_management: {e}")
    ...
    def show_add_staff_dialog(self):
        ...
        username_label = QLabel("Username:")
        username_label.setStyleSheet("color: white; font-size: 14px;")
        self.add_staff_username = QLineEdit()
        self.add_staff_username.setStyleSheet("background-color: white; border-radius: 5px; color: black; font-size: 14px; padding: 5px;")
        self.add_staff_username.returnPressed.connect(lambda: self.add_staff_password.setFocus())
        
        password_label = QLabel("Password:")
        password_label.setStyleSheet("color: white; font-size: 14px;")
        self.add_staff_password = QLineEdit()
        self.add_staff_password.setEchoMode(QLineEdit.EchoMode.Password)
        self.add_staff_password.setStyleSheet("background-color: white; border-radius: 5px; color: black; font-size: 14px; padding: 5px;")
        self.add_staff_password.returnPressed.connect(lambda: self.add_staff_email.setFocus())
        
        email_label = QLabel("Email:")
        email_label.setStyleSheet("color: white; font-size: 14px;")
        self.add_staff_email = QLineEdit()
        self.add_staff_email.setStyleSheet("background-color: white; border-radius: 5px; color: black; font-size: 14px; padding: 5px;")
        self.add_staff_email.returnPressed.connect(lambda: confirm_button.click())
        
        form_layout.addWidget(username_label, 0, 0)
        form_layout.addWidget(self.add_staff_username, 0, 1)
        form_layout.addWidget(password_label, 1, 0)
        form_layout.addWidget(self.add_staff_password, 1, 1)
        form_layout.addWidget(email_label, 2, 0)
        form_layout.addWidget(self.add_staff_email, 2, 1)
        ...
        """)
        confirm_button.clicked.connect(lambda: self.handle_add_staff(dialog))
        ...
        self.add_staff_username.setFocus()
        dialog.exec()

    def handle_add_staff(self, dialog):
        username = self.add_staff_username.text().strip()
        password = self.add_staff_password.text().strip()
        email = self.add_staff_email.text().strip()
    ...
    def clear_order(self):
        try:
            ...
            print(f"Error in clear_order: {e}")
